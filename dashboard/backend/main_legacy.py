...
- 클러스터 상태 조회
- 노드 리소스 모니터링 (CPU/메모리 사용률)
- GPU 상세 정보 (온도, VRAM, 사용률)
- Pod 목록 및 상태
- 워크로드 관리 (vLLM, RustFS, Qdrant)
- 임베딩 서비스 (sentence-transformers)
"""
...
async def control_workload(workload_name: str, action: WorkloadAction):
    ...
    try:
        ...
        if e.status == 404:
            raise HTTPException(
                status_code=404,
                detail=f"{workload_name}이 아직 배포되지 않았습니다. 먼저 배포해주세요."
            )
        raise HTTPException(status_code=500, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
        ...
        for pod in pods.items:
            ...
            for container in (pod.spec.containers or []):
                ...
                try:
                    ...
                    node_gpu_usage[node_name] += gpu_count
                except:
                    pass
...
async def get_storage_usage_breakdown():
    """스토리지 사용량 분류 조회 (파일 스토리지, RustFS, 시스템 등)"""
    try:
        import subprocess
...
async def generate_comfyui_image(request: dict):
    """Queue an image generation"""
    import uuid
    from datetime import datetime

    workflow_id = request.get("workflow_id", "txt2img_basic")
    ...
    # In real implementation, this would queue to ComfyUI
    return {
